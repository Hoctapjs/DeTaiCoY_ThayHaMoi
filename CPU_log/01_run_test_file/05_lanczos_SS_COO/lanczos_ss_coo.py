...
def matrix_vector_product(A, v):
    ...
    return A @ v # nhân từ dòng nào trong ma trận với vector
...
def Lanczos(A, v, m):
    ...
    # Sử dụng phiên bản tuần tự thay vì song song cho một tác vụ duy nhất
    try:
        w = A @ V[0, :]  # Nhân ma trận A với V[0, :] trực tiếp
    except Exception as e:
        logging.error(f"Error in matrix-vector product: {e}")
        raise

    # Sử dụng joblib để tính alpha (giữ nguyên vì có thể có lợi nếu mở rộng)
    try:
...
def compute_eigen(L, D, k=2):
    ...
    """
    # Chuẩn hóa ma trận Laplace: D^-1/2 * L * D^-1/2
    D_diag = D.diagonal().copy()  # Lấy đường chéo của D
    D_diag[D_diag < 1e-10] = 1e-10  # Tránh chia cho 0 hoặc gần 0
    D_inv_sqrt = diags(1.0 / np.sqrt(D_diag))  # Tính D^-1/2
    L_normalized = D_inv_sqrt @ L @ D_inv_sqrt  # Chuẩn hóa ma trận Laplace
