...
def parallel_matvec(A, v, n_jobs=4):
    """Tính tích ma trận - vector song song"""
    n_rows = A.shape[0]
    chunk_size = (n_rows + n_jobs - 1) // n_jobs  # Chia thành các phần nhỏ
    results = Parallel(n_jobs=n_jobs)(
        delayed(lambda start, end: A[start:end] @ v)(i, min(i + chunk_size, n_rows))
        for i in range(0, n_rows, chunk_size)
    )
    return np.concatenate(results).reshape(-1)  # Đảm bảo kết quả là vector 1D

def parallel_dot(a, b, n_jobs=4):
    """Tích vô hướng hai vector song song"""
    n = len(a)
    chunk_size = (n + n_jobs - 1) // n_jobs
    partial_sums = Parallel(n_jobs=n_jobs)(
        delayed(lambda start, end: np.dot(a[start:end], b[start:end]))(i, min(i + chunk_size, n))
        for i in range(0, n, chunk_size)
    )
    return sum(partial_sums)

def Lanczos_parallel(A, v, m, n_jobs=4):
    """Phiên bản song song của thuật toán Lanczos"""
    n = len(v)
    V = np.zeros((m, n))
    T = np.zeros((m, m))
    ...
    start = time.time()
    w = parallel_matvec(A, V[0, :], n_jobs)  # Nhân ma trận - vector song song
    end = time.time()
    logging.info(f"Thoi gian nhan ma tran - vector (song song): {end - start:.6f} giay")

    start = time.time()
    alpha = parallel_dot(w, V[0, :], n_jobs)  # Tích vô hướng song song
    end = time.time()
    ...
    for j in range(1, m):
        ...
        if beta < 1e-10:
            break
        V[j, :] = w / beta
        w = parallel_matvec(A, V[j, :], n_jobs)
...
def compute_eigen(L, D, k=2):
    ...
    T, V = Lanczos_parallel(L_normalized, v0, m=k+5)  # Sử dụng m > k để tăng độ chính xác
    # end
