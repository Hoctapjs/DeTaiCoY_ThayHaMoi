...
def parallel_dot(a, b, n_jobs=4):
    """Tích vô hướng song song nhưng tối ưu hơn"""
    chunk_size = len(a) // n_jobs
    partial_sums = Parallel(n_jobs=n_jobs, prefer="threads")(
        delayed(np.dot)(a[i:i+chunk_size], b[i:i+chunk_size])
        for i in range(0, len(a), chunk_size)
    )
    return sum(partial_sums)

def Lanczos_parallel(A, v, m, n_jobs=4):
    """Phiên bản song song tối ưu của thuật toán Lanczos"""
    n = len(v)
    V = np.zeros((m, n))
    T = np.zeros((m, m))
    ...
    start = time.time()
    w = A @ V[0, :]  # Nhân ma trận - vector tối ưu
    end = time.time()
    logging.info(f"Thoi gian nhan ma tran - vector: {end - start:.6f} giay")

    start = time.time()
    alpha = parallel_dot(w, V[0, :], n_jobs)  # Dùng dot song song tối ưu
    end = time.time()
    logging.info(f"Thoi gian tinh tich vo huong: {end - start:.6f} giay")
    ...
    for j in range(1, m):
        ...
        V[j, :] = w / beta
        w = A @ V[j, :]
        alpha = parallel_dot(w, V[j, :], n_jobs)
...
def compute_eigen(L, D, k=2):
    """Giải bài toán trị riêng bằng thuật toán Lanczos song song tối ưu"""
    # Chuan hoa ma tran Laplace
    D_diag = D.diagonal().copy()
    D_diag[D_diag < 1e-10] = 1e-10  # Tranh chia cho 0
    D_inv_sqrt = diags(1.0 / np.sqrt(D_diag))
    L_normalized = D_inv_sqrt @ L @ D_inv_sqrt  # Chuan hoa ma tran

    # Khởi tạo vector ngẫu nhiên
    v0 = np.random.rand(L.shape[0])
    v0 /= np.linalg.norm(v0)

    # Áp dụng thuật toán Lanczos song song
    T, V = Lanczos_parallel(L_normalized, v0, m=k+5)
