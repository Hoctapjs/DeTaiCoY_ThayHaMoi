...
# ĐÂY LÀ PHIÊN BẢN TỐI ƯU ĐỂ CHẠY ẢNH LỚN 200X200 KO NỔI 
...
# Tính ma trận trọng số thưa (tối ưu hóa bộ nhớ)
def compute_weight_matrix(image, sigma_i=0.1, sigma_x=10, threshold=1e-5, chunk_size=1000):
    h, w, c = image.shape
    n = h * w
    coords = np.array(np.meshgrid(range(h), range(w))).reshape(2, -1).T
    features = image.reshape(-1, c)
    
    # Khởi tạo danh sách để lưu trữ dữ liệu thưa
    rows = []
    cols = []
    data = []
    
    # Xử lý từng chunk để giảm bộ nhớ
    for i in range(0, n, chunk_size):
        i_end = min(i + chunk_size, n)
        chunk_features = features[i:i_end]
        chunk_coords = coords[i:i_end]
        
        # Tính khoảng cách cho chunk hiện tại với toàn bộ tập dữ liệu
        dist_features = np.sum((chunk_features[:, np.newaxis, :] - features[np.newaxis, :, :])**2, axis=2)
        dist_coords = np.sum((chunk_coords[:, np.newaxis, :] - coords[np.newaxis, :, :])**2, axis=2)
        
        # Tính W cho chunk
        W_chunk = np.exp(-dist_features / (2 * sigma_i**2)) * np.exp(-dist_coords / (2 * sigma_x**2))
        
        # Lọc các giá trị lớn hơn threshold
        chunk_rows, chunk_cols = np.nonzero(W_chunk > threshold)
        chunk_data = W_chunk[chunk_rows, chunk_cols]
        
        # Điều chỉnh chỉ số hàng cho chunk
        chunk_rows = chunk_rows + i
    ...
    # Chuyển thành ma trận COO
    W_sparse = coo_matrix((data, (rows, cols)), shape=(n, n))
    return W_sparse
...
def compute_laplacian(W_sparse):
    ...
    D = diags(D_diag)
    L = D - W_sparse  # Giữ thưa
    return L, D

# Tích vô hướng với Numba
@jit(nopython=True)
def handle_dot(a, b):
    result = 0.0
...
# Nhân ma trận-vector tối ưu hóa
@jit(nopython=True)
...
# Lanczos cho ma trận thưa (Numba)
@jit(nopython=True)
...
def compute_eigen(L, D, k=2):
    ...
    if n < 10000:
        ...
        T, V = lanczos_dense(L_normalized.toarray(), v0, m=k+5)
